...
This is expected behavior. We store the variation displayed to each visitor in their browser’s local storage for a period of 7 days. This mechanism helps avoid repeatedly querying our database each time a visitor returns to your site. This not only conserves resources but also ensures a smoother and faster user experience.
...
However, a consequence of this approach is that even if an experiment is paused, the stored variation remains active for the 7-day period. This means that visitors, including yourself, will continue to see the same variation regardless of the experiment’s current status.
