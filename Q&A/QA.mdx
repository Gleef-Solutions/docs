...
Gleef allows you to run multiple experiments simultaneously. However, conducting several experiments on the same page will impact the time required to reach statistical significance.

To prevent any bias in the experimentation process, if multiple experiments are active on the same page (same URL), each visitor will be randomly assigned to a single experiment. This approach ensures that there is no interference bias between the different experiments.

Although this method maintains the integrity of each experiment, it will extend the time needed to achieve statistical significance. Therefore, it is advisable to complete one experiment before starting another to minimize delays and ensure accurate results.